...
btn.addEventListener('click', () => {
    ...
    const formattedContent = editorContent.replace(/<p[^>]+>/g, "<p>").replace("<p>TRANSCRIPT</p>",'<hr class="my-2x"/><h2>Transcript</h2>').replaceAll("<strong>",'<strong class="sans-serif font-weight-bold">').replace(/\d:\d\d:\d\d\.\d/g,'<span class="text-sm text-dark-gray">$&</span><br/>').replaceAll("<p>&nbsp;</p>","").replaceAll(/\[.+?\]/g, '<span class="sans-serif font-weight-bold text-sm letter-spacing-default text-dark-gray">$&</span>');
